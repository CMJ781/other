...
sed -i 's/by/Ipv4P Mini S$(TZ=UTC-8 date +%Y.%m.%d) by'  ./package/base-files/files/etc/banner
...
#sed -i "Ipv4P Mini s/${aa}/${aa}-$(TZ=UTC-8 date +%Y.%m.%d) by Sirpdboy/g" package/base-files/files/etc/openwrt_release
